...
//     this is the method which is used to store data in the array list
//     we return an arraylist from this method

    public static ArrayList<Earthquake_items> extractEarthquakeData(){

//     here we are creating an arraylist which will be accessed with this method

        ArrayList<Earthquake_items> arraylist = new ArrayList<>();

//        this to handle the exception which will arise if getJSONObject does not find any value at the given key
       try{

//        /** we create an object for the root key */
           JSONObject root = new JSONObject(JSON_DATA_STRING);

//           /** using the root object we access the "features" key through an object */
           JSONArray feat = root.getJSONArray("features");

//           /** access each element of the array  */
           for(int i=0;i<feat.length();i++){

//               /** creating an object to access elements present at a particular index of the array */
                JSONObject feat_elements = feat.getJSONObject(i);

//               /** object to access the value in properties key */
...
//               /** object to access the time key */
//               we use long datatype because Date constructor takes long type argument
//               and if we want to take sum or average of some kind then we can easily do operations on long
                long day = prop.getLong("time");

//               /** setting value in the array list */
                arraylist.add(new Earthquake_items(magnitude,place,day));
           }

       }catch( JSONException e){
           Log.e("QueryUtilis","Problem parsing the JSON data",e);
       }
...
// NOTE:
// we observe that while parsing an object or Array we use getJSONObject or getJSONArray methods
// when we need to take out just the value then we use getString, getLong etc methods
